...
# This is equivalent to 0(n^2) due to dropping the second, non-dominant code block
...
# The reason that you would use a linked list over lists is because there are things that a linked list # can do more efficiently. For example, adding and removing from the beginning of a linked list is O(1) # while adding and removing from the beginning of a list is O(n).

# Think of data structures like a black box. You can put items in. You can take items out. If removing # the last item added quickly is your top priority and you have three choices which one would you choose?:

# 1) O(n)
# 2) O(log n)
# 3) O(1)

# It does not matter what data structure is in the black box, you know that you want the O(1) solution. # Here is what is in each black box:

# 1) O(n) Linked List
# 2) O(log n) Binary Search Tree
# 3) O(1) Lists or Doubly Linked List

# Now we have narrowed things down to two data structures. 
# Let's say our second priority is removing items from the front of the list.  You have two options:

# 1) O(1)
# 2) O(n)

# Once again you want the O(1) option.  Now you have narrowed the proper data structure down to Doubly  # Linked Lists. It is going to be O(1) for both of your top priorities.

# Picking the best data structure is almost always about Big O.

# There will be other situations where knowing the relationships between items in the data structure is # important.  This is where a graph work better than anything and Big O will not give you the answer.
