...
# recursively call the helper function on the left child of the node and store
# the result in left_result    
    left_result = self.kth_smallest_helper(node.left, k)
    if left_result is not None:
...
# recursively call the helper function on the right child of the node and store
# the result in right_result    
    right_result = self.kth_smallest_helper(node.right, k)
    if right_result is not None:
